...
| Метод | Путь | Роль | Описание |
|---|---|---|---|
| POST | `/payments` | GUEST | Инициировать оплату бронирования (только своего) |
...
| POST | `/payments/{id}/refund` | GUEST (свой), ADMIN | Запрос возврата |
| POST | `/payments/{id}/retry` | GUEST (свой) | Повторная попытка оплаты |
| GET | `/refunds/{id}` | GUEST (свой), ADMIN | Статус возврата |
| POST | `/refunds/{id}/retry` | GUEST (свой) | Повторная попытка возврата |
### 3.4 review-service
